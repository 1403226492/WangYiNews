...
public class WangYi {
    ...
    //根据内容url2获取新闻信息并进行存储
    private static void getContent(String url2, List<String> ids,String type) {
        ...
        for (;i<ids.size();i++){
            ...
            try {
                ...
                try {
                    dbCollection = MongoDBUtil.connMongoDB();
                } catch (Exception e) {
                    e.printStackTrace();
                }
                BasicDBObject obj = new BasicDBObject();
                obj.put("type",type);
                obj.put("title", title.html());
